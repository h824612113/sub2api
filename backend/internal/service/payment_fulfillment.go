...
func (s *PaymentService) ensurePaymentSubscriptionAssigned(ctx context.Context, o *dbent.PaymentOrder, groupID int64, days int) error {
	...
	txClient := tx.Client()
	bundleGroupIDs, err := s.subscriptionSvc.resolveSubscriptionBundleGroupIDs(txCtx, groupID)
	if err != nil {
		return fmt.Errorf("resolve subscription bundle: %w", err)
	}
	...
	if !alreadyAssigned {
		...
		for _, bundleGroupID := range bundleGroupIDs {
			existing, lookupErr := s.subscriptionSvc.userSubRepo.GetByUserIDAndGroupID(txCtx, o.UserID, bundleGroupID)
			switch {
		...
		if _, err := txClient.PaymentAuditLog.Create().
			...
			if dbent.IsConstraintError(err) {
				...
				if checkErr == nil && claimed {
					return s.invalidatePaymentSubscriptionBundleCaches(o.UserID, bundleGroupIDs)
				}
			}
	...
	if err := s.invalidatePaymentSubscriptionBundleCaches(o.UserID, bundleGroupIDs); err != nil {
		return fmt.Errorf("invalidate subscription cache after fulfillment: %w", err)
	}
	return nil
}

func (s *PaymentService) invalidatePaymentSubscriptionBundleCaches(userID int64, groupIDs []int64) error {
