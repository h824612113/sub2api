...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	finalResponse, usage, usageIncompleteReason, acc, err := s.readOpenAICompatBufferedTerminal(resp, c, "openai messages buffered", requestID)
	if err != nil {
					...
					if err := json.Unmarshal([]byte(payload), &event); err == nil {
						...
						if response := openAICompatTerminalResponse(&event, []byte(payload)); isOpenAICompatResponsesTerminalEvent(event.Type) && response != nil {
							if response.Usage != nil {
								...
								return response, usage, "terminal event missing usage", acc, nil
							}
...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		if isTerminalEvent {
			...
			if event.Response != nil && event.Usage == nil && event.Response.Usage == nil {
