...
func TestForwardAsChatCompletions_TerminalWithoutUsageMarksUsageIncomplete(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_missing_usage"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.Zero(t, result.Usage.InputTokens)
	require.Zero(t, result.Usage.OutputTokens)
	require.Equal(t, "terminal event missing usage", result.UsageIncompleteReason)
	require.Contains(t, rec.Body.String(), "data: [DONE]")
}

func TestForwardAsChatCompletions_BufferedTerminalWithoutUsageMarksUsageIncomplete(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_buffered_missing_usage"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
