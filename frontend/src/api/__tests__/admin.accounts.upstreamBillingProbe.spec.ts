...
describe('admin account upstream billing probe API', () => {
  ...
  it('reads and updates automatic account probe settings', async () => {
    const settings = { enabled: true, interval_minutes: 5, auto_recover: true }
    get.mockResolvedValueOnce({ data: settings })
    put.mockResolvedValueOnce({ data: settings })

    await expect(getAccountAutoProbeSettings()).resolves.toEqual(settings)
    await expect(updateAccountAutoProbeSettings(settings)).resolves.toEqual(settings)
    expect(get).toHaveBeenCalledWith('/admin/accounts/auto-probe/settings')
    expect(put).toHaveBeenCalledWith('/admin/accounts/auto-probe/settings', settings)
  })
})
